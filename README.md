...
```
  📁 /data/
    ...
    │   └── 2025년2월_매출.csv        카탈로그에 등록됨과 함께 내용을 db에 적재합니다.
    ├── 보고서/
    │   ├── 시장분석_2025Q1.pdf     ← PDF, 한글, Word 넣으면
    │   └── 기술검토_보안.hwp          카탈로그에 등록됩니다.
    └── 로그/
...
> **기존 환경을 건드리지 않습니다.** 공유 폴더도 그대로, FTP 서버도 그대로, cron도 그대로.옆에 서버 하나 설치하는 것뿐입니다.
...
아래 작업은 에이전트가 SQL을 생성한 뒤, **관리자가 직접 확인하고 승인**해야 실행됩니다.
...
> 완전 폐쇄망이 아니고, 인터넷이 연결된 환경이라면 상용 모델을 활용하는 것도 가능합니다.
...
하지만 인터넷이 가능한 환경이라면 상용 모델을 활용하는 것도 가능합니다.
...
현재 시스템은 관리자가 SQL이나 인프라 구조를 어느 정도 이해한다는 전제하에 설계되었습니다. 
모르더라도 운영 자체는 가능하겠지만, 문제가 생겼을 때 해결하기는 어려울 수 있습니다.
